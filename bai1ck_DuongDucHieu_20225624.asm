...
.eqv KEY_f 0x88
# eqv for Keyboard
.eqv IN_ADRESS_HEXA_KEYBOARD 0xFFFF0012 #Địa chỉ đầu vào của bàn phím
.eqv OUT_ADRESS_HEXA_KEYBOARD 0xFFFF0014 #Địa chỉ đầu ra của bàn phím
...
addi $sp, $sp, 4 # backup: de khong bát thay doi gia tri khi lay ra thuc hien
sw $t1, 0($sp)
addi $sp, $sp, 4
sw $t2, 0($sp)
addi $sp, $sp, 4
...
GO:
addi $sp, $sp, 4 # backup
sw $at, 0($sp) # lưu gtri thanh ghi $at vào vtri 0(đchi) của năng epes
addi $sp, $sp, 4
sw $k0, 0($sp)
li $at, MOVING # MOVING == 1 -> DI CHUYEN
addi $k0, $zero, 1
sb $k0, 0($at) #đối tượng đang di chuyển
